...
const USE_REAL_RECOGNITION = false; // Mantener reconocimiento en mock por ahora
...
export class DataProvider {
  ...
  /**
   * Reconoce productos desde una imagen
   * Mock mejorado que simula comportamiento realista de IA
   */
  async recognize(image: File | string): Promise<{ items: RecognizedItem[] }> {
    if (USE_REAL_RECOGNITION) {
      ...
      const response = await fetch(`${API_BASE_URL}/recognition/recognize`, {
        method: 'POST',
        body: formData,
      });
      
      if (!response.ok) throw new Error('Error en reconocimiento');
  ...
  async createTransfer(transfer: Omit<Transfer, 'id' | 'fechaISO' | 'estado'>): Promise<Transfer> {
    if (USE_REAL_API) {
      const backendPayload = {
        origenId: transfer.depositoOrigenId,
        destinoId: transfer.depositoDestinoId,
      ...
      return {
        id: result.id,
        fechaISO: result.fechaISO,
        depositoOrigenId: result.origenId,
        depositoDestinoId: result.destinoId,
        items: result.items,
        estado: 'completado'
      };
    }
  ...
  async getTransfers(): Promise<Transfer[]> {
    if (USE_REAL_API) {
      ...
      return transfers.map((t: any) => ({
        id: t.id,
        fechaISO: t.fecha_iso,
        depositoOrigenId: t.origen_id,
        depositoDestinoId: t.destino_id,
        items: t.items || [],
        estado: 'completado'
      }));
    }
  ...
  async getTransfer(id: string): Promise<Transfer | null> {
    if (USE_REAL_API) {
      ...
      return {
        id: transfer.id,
        fechaISO: transfer.fecha_iso,
        depositoOrigenId: transfer.origen_id,
        depositoDestinoId: transfer.destino_id,
