...
import { FileBarChart, Download, Calendar, TrendingUp, Receipt, Package, Filter, FileText, ChevronDown } from 'lucide-react';
...
import { exportToCSV, exportToPDF, formatDataForExport, formatters } from '@/utils/exportUtils';
...
const ReportsPage: React.FC = () => {
  ...
  const exportSalesCSV = () => {
    try {
      ...
      const csvData = filteredTickets.flatMap(ticket => 
        ticket.items.map(item => ({
          'ID Ticket': ticket.id,
          'Fecha': format(new Date(ticket.fechaISO), 'dd/MM/yyyy HH:mm'),
          'Depósito': getDepositName(ticket.depositoId),
          'SKU': item.sku,
          'Producto': item.nombre,
          'Cantidad': item.qty,
          'Precio Unitario': `$${item.precioAplicado.toFixed(2)}`,
          'Subtotal': `$${item.subtotal.toFixed(2)}`,
          'Total Ticket': `$${ticket.total.toFixed(2)}`
        }))
      );

      exportToCSV(csvData, { 
        filename: `ventas_${dateFrom}_${dateTo}.csv`,
  ...
  const exportSalesPDF = () => {
    try {
      ...
      const pdfData = filteredTickets.flatMap(ticket => 
        ticket.items.map(item => ({
          'ID Ticket': ticket.id,
          'Fecha': format(new Date(ticket.fechaISO), 'dd/MM/yyyy HH:mm'),
          'Depósito': getDepositName(ticket.depositoId),
          'Producto': item.nombre,
          'Cantidad': item.qty,
          'Precio Unit.': `$${item.precioAplicado.toFixed(2)}`,
          'Subtotal': `$${item.subtotal.toFixed(2)}`,
      ...
      exportToPDF(pdfData, { 
        filename: `ventas_${dateFrom}_${dateTo}.pdf`,
        title: 'Reporte de Ventas',
  ...
  const exportStockCSV = () => {
    try {
      ...
      if (filteredMovements.length === 0) {
        toast({ 
          title: 'Sin datos', 
          description: 'No hay movimientos de stock para exportar con los filtros actuales',
          variant: 'destructive'
        });
        return;
      }
      ...
      const csvData = filteredMovements.map(movement => ({
        ...
        'Depósito': getDepositName(movement.depositoId),
      ...
      exportToCSV(csvData, { 
        filename: `stock_movimientos_${dateFrom}_${dateTo}.csv`,
  ...
  const exportStockPDF = () => {
    try {
      ...
      const pdfData = filteredMovements.map(movement => ({
        'ID': movement.id,
        'Fecha': format(new Date(movement.fechaISO), 'dd/MM/yyyy HH:mm'),
        'Depósito': getDepositName(movement.depositoId),
        'Producto': movement.producto || movement.productId,
      ...
      exportToPDF(pdfData, { 
        filename: `stock_movimientos_${dateFrom}_${dateTo}.pdf`,
        title: 'Reporte de Movimientos de Stock',
        subtitle: `Período: ${format(new Date(dateFrom), 'dd/MM/yyyy')} - ${format(new Date(dateTo), 'dd/MM/yyyy')}`
      });
      toast({ title: 'PDF generado exitosamente' });
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Promedio por Ticket</CardTitle>
            <Receipt className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              ${kpis.cantidadTickets > 0 ? (kpis.totalVentas / kpis.cantidadTickets).toFixed(2) : '0.00'}
            </div>
            <p className="text-xs text-muted-foreground">
